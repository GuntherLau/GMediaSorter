...
- ▶️ **视频预览/播放**：原生播放器支持倍速、音量、循环、全屏与快捷键
- 🧩 **动态视频拼墙**：将当前过滤结果以瀑布流形式全屏播放，支持性能预设与单源音频轮换

## 动态视频拼墙

### 入口与开启

- 在主界面工具栏点击“动态视频拼墙”按钮，当前过滤结果存在视频时入口才会启用。
- 点击按钮后会先弹出“动态视频拼墙配置”对话框，可选择列数与性能预设；默认值沿用上次的设定。
- 首次进入会自动请求全屏权限，以便获得沉浸式播放体验。
- 如果全屏请求被拒绝，可手动按 `F` 或使用系统快捷键重新进入全屏。

### 布局与控制

- 在配置对话框中调整“拼墙列数”和“性能预设”后再进入拼墙，列数越高意味着更紧凑的网格。
- 预设包含低（6 路）、中（12 路）、高（18 路）三档，同时控制洗牌批次与预加载深度。
- 每个视频瓦片会根据宽高比自动计算网格跨度，实现无缝拼贴的瀑布流效果。
- 拼墙界面本身不提供额外控件，按 `Esc` 或退出全屏即可返回主界面。

### 音频与调度

- 任何时刻仅有一个瓦片输出音频，应用会按固定间隔在当前活跃瓦片间轮换声音焦点。
- 播放失败的源会进入冷却队列，在 45 秒内不会再次尝试，避免重复报错。
- 待播队列会自动预加载，补位时可即刻切换，减少首帧等待。

### 常见问题

- **入口不可用**：确认当前过滤结果中至少存在一个视频文件。
- **全屏失败**：检查系统是否允许窗口置顶或全屏，必要时手动触发全屏。
- **卡顿或掉帧**：在拼墙配置对话框中切换到“低”性能预设或减少列数，减轻每帧渲染压力。
- **音频缺失**：部分素材无音轨，或全屏权限被系统限制（如浏览器嵌入模式）。

## 多维过滤面板

- 主界面左上角提供分辨率、时长与长宽比三大过滤维度，可叠加组合实现精准筛选。
- 长宽比支持“竖屏”“方形”“标准横屏”“宽屏/电影”等常用档位，并提供“仅查看未知”快捷按钮，方便快速定位缺失宽高信息的素材；再次点击按钮会恢复到进入快捷筛选前的选择。
- 过滤摘要会实时显示命中数量及当前条件，当存在未知长宽比素材时会提示“含 X 个未知长宽比”，便于后续补录元数据。

## 视频预览/播放

### 入口与触发

- 在视频列表、重复结果或相似结果面板中，点击卡片右上角的“播放”按钮即可打开播放器。
- 播放器以侧边面板形式呈现，保持列表可见，便于对比与筛选。

### 播放器功能

- **播放控制**：播放/暂停、进度条拖拽、循环模式切换。
- **音量与静音**：支持静音按钮与音量滑块，键盘上下键快速调整音量。
- **倍速播放**：可选择 0.5x、0.75x、1x、1.25x、1.5x、2x 等档位。
- **全屏模式**：点击全屏按钮或按下 F 键进入/退出全屏。
- **元数据信息**：展示编解码格式、容器类型、分辨率、平均码率等信息。
...
- 播放器会记住最近一次的自动播放、循环和默认倍速设置。
- 如需重置偏好，可在面板顶部的设置区域进行调整。

### 常见问题

- **无法播放**：确认文件仍然存在且格式受支持；若路径失效，可重新扫描目录。
- **无声音**：检查系统音量或确认视频自带音轨；部分视频可能是无声素材。
- **播放卡顿**：高清高码率文件在低性能设备上可能出现卡顿，可尝试降速或暂停其他任务。
